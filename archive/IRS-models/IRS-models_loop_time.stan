...
model {
  ...
  for (n in 1:N) {
      ...
      for (i in 1:m) {
          Xfs[i] ~ Bin(M[i,n], pfs[n])
              logit(pfs[n]) = alpha1 + alpha2 * time[n];
         }

      for (i in 1:r) {
         Xd[i] ~ Bin(R[i,n], pd[n])
             logit(pd[n]) = omega1 + omega2 * time[n];
        }

     for (i in 1:k) {
        Xufs[i] ~ Bin(K[i,n], pufs[n])
            logit(pufs[n]) = beta1 + beta2 * time[n];
       }
